{%- from "formatting.jinja" import yaml_string, systemd_escape, sha256sum, trim_common_whitespace %}
...
{%- set common_whitespace_tests = {
      '': '',
      'first\n   second\n   third': 'first\nsecond\nthird',
      '   first\n     second\n   third': 'first\n  second\nthird' } %}

{%- set yaml_string_tests = {
    '': '""',
    '\nhello': '"\\nhello"',
    'no prefix trimming\n     should be the same': '"no prefix trimming\\n     should be the same"' } %}
...
{%- for test, expected in common_whitespace_tests.items() %}
{{p}}common_whitespace value test {{ loop.index }}:
  {{- test_eq(test, expected, trim_common_whitespace(test)) }}
{{p}}common_whitespace block test {{ loop.index }}:
  {%- set actual %}
    {%- call trim_common_whitespace() %}
      {{- test }}
    {%- endcall %}
  {%- endset %}
  {{- test_eq(test, expected, actual) }}
...
{%- for test, expected in yaml_string_tests.items() %}
{{p}}yaml_string value test {{ loop.index }}:
  {{- test_eq(test, expected, yaml_string(test)) }}
{{p}}yaml_string block test {{ loop.index }}:
  {%- set actual %}
    {%- call yaml_string() %}
      {{- test }}
    {%- endcall %}
